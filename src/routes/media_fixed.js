...
// GET /media/liked - Get user's liked media (must come before :id route)
...
// GET /media/search/suggestions - Get search suggestions for autocomplete
router.get('/search/suggestions', async (req, res) => {
  try {
    ...
    const titleSuggestions = await pool.query(
      `SELECT DISTINCT m.title, 'title' as type
       FROM media m 
       WHERE m.is_approved = true AND m.title ILIKE $1
       LIMIT $2`,
      [`%${searchTerm}%`, Math.floor(limit / 3)]
    );

    const artistSuggestions = await pool.query(
      `SELECT DISTINCT m.artist, 'artist' as type
       FROM media m 
       WHERE m.is_approved = true AND m.artist ILIKE $1 AND m.artist IS NOT NULL AND m.artist != ''
       LIMIT $2`,
      [`%${searchTerm}%`, Math.floor(limit / 3)]
    );

    const categorySuggestions = await pool.query(
      `SELECT DISTINCT m.category, 'category' as type
       FROM media m 
       WHERE m.is_approved = true AND m.category ILIKE $1 AND m.category IS NOT NULL AND m.category != ''
       LIMIT $2`,
      [`%${searchTerm}%`, Math.floor(limit / 3)]
    );

    const creatorSuggestions = await pool.query(
      `SELECT DISTINCT u.full_name, 'creator' as type
       FROM media m 
       JOIN users u ON m.uploader_email = u.email
       WHERE m.is_approved = true AND u.full_name ILIKE $1
       LIMIT $2`,
      [`%${searchTerm}%`, limit - titleSuggestions.rows.length - artistSuggestions.rows.length - categorySuggestions.rows.length]
    );

    const allSuggestions = [
      ...titleSuggestions.rows.map(row => ({ title: row.title, type: row.type })),
      ...artistSuggestions.rows.map(row => ({ title: row.artist, type: row.type })),
      ...categorySuggestions.rows.map(row => ({ title: row.category, type: row.type })),
      ...creatorSuggestions.rows.map(row => ({ title: row.full_name, type: row.type }))
    ];
